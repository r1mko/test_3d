...
public class GroundPlatform : MonoBehaviour
{
    ...
    private IEnumerator PlaySequentialJump(Hexagon hex, [Bridge.Ref] Vector3 worldTargetPos, [Bridge.Ref] Vector3 localTargetPos, float delay, Action onJumpFinished, float speedMultiplier = 1.0f)
    {
        yield return new WaitForSeconds(delay);
